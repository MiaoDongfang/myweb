...
def _static_file_generator(fpath):   #什么作用
    BLOCK_SIZE=8192
...
class Request(object):
        ...
        if not hasattr(self,'_cookies'):
            ...
            if cookie_str:
                for c in cookie_str.split(';'):
                    pos=c.find('=')
                    if pos>0:
                        cookies[c[:pos]]=_unquote(c[pos+1:])
    ...
    def cookie(self,cookie,default=None):
        return self._get_cookies().get(cookie,default)
...
class Response(object):
    ...
    def status_code(self,value):
        if isinstance(value,(int,long)):
            ...
            raise TypeError("Bad type of response status code")
...
class Jinja2TemplateEngine(TemplateEngine):
    ...
    def __init__(self,templ_dir,**kw):
        #Environment定义了  类似{{   }} {%  %}的语法
        #FileSystemLoder  加载文件系统中模板的类
...
def view(path):
    def _decorator(func):
        ...
        def _wrapper(*args,**kw):
            ...
            raise ValueError("Expect return a dict when usins @view() decorator.")
        return _wrapper
    return _decorator
...
#         if self._running:
#             raise RuntimeError("Cannot modify WSGIApplication when runing")
#
#     @property
